...
export default function DeclararPropietario() {
  ...
  const agregarItem = async () => {
      ...
      let numCode
      let aux = JSON.parse(JSON.stringify(propietarios))
      let temp = JSON.parse(JSON.stringify(propietarios))
      if (nombre !== "" ) {
        ...
        CREAR RESPONSABLE
          ...
          <Grid item xs={12} md={2}>
              ...
              <Grid item xs={12} md={7}>
                  <TextField id="outlined-basic" inputProps={{ style: { textTransform: "uppercase" } }} value={nombre} error={false} fullWidth label="PROPIETARIO" variant="outlined" onChange={(event) => { setNombre(event.target.value) }} />
              </Grid>
              {/* <Grid item xs={12} md={2}>
              ...
              <Grid item xs={12} md={3}>
                  <Button variant="contained" sx={{ height: "100%", width: "100%" }} onClick={agregarItem}>Agregar Propietario</Button>
              </Grid>
              <Grid item xs={12} md={12}>
