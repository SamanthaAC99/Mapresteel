...
export default function IndicadoresOT() {
    ...
    const handleOrdenes =()=>{
        ...
        const fechasObtenidas = departamentoFilter.map(orden => orden)

        setLonordenes(departamentoFilter.length)
        ...
        setPendientes(departamentoFilter.filter(filterStatePendientes).length)
    ...
    const filterbyarea = (state) =>{
        if (state.departamento  === departamento ){
            return state
        } else if (departamento === 'ALL') {
            return state
        } else {
            return
        }
    ...
    const filterStatePendientes = (state) => {
        if (state.estado === "Iniciada" || state.estado === "Pendiente" ) {
            return state;
        } else {
            return
        }
    ...
    const filterTipoPendientesSuspendida = (state) => {
        if (state.razonp === "Suspendida") {
            return state;
        } else {
            return
        }
                ...
                <Grid container spacing={3}>
                    ...
                    <Grid item xs={12} sm={12} md={3}>
                        <div className="card-container">
                            <TarjetaIndicadores icono={<SummarizeIcon/>} valor={lonordenes} bgicon={blue[700]} titulo={'N° Ordenes Totales'} colort={"#598ec7"} />
                        </div>
                    </Grid>
                    <Grid item xs={12} sm={12} md={3}>
                        <div className="card-container">
                            <TarjetaIndicadores icono={<PlayArrowIcon/>} valor={activas} bgicon={blue[700]} titulo={'N° Ordenes Activas'} colort={"#598ec7"} />
                        </div>
                    </Grid>
                    <Grid item xs={12} sm={12} md={3}>
                        <div className="card-container">
                    ...
                    <Grid item xs={12} sm={12} md={3}>
                        <div className="card-container">
                            <TarjetaIndicadores icono={< PendingActionsIcon />} valor={pendientes} bgicon={blue[700]} titulo={'N° Ordenes Pendientes Totales'} colort={"#598ec7"} />
                        </div>
                    </Grid>
                        ...
                        <div className="card12" >

                            {
                                <div className="card-body12 small ">
                                    ...
                                    <BarChart labels={['Ene', 'Feb', 'Mar', 'Apr', 'May', 'Jun', 'Jul', 'Ago', 'Sep', 'Oct', 'Nov', 'Dec']} datos={tiempoMedioR} />
                                </div>
                            }
